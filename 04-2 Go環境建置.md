...
1. Git(2.26.2) 2020/05/29 [官方網站](https://git-scm.com/download/win)
2. Git bash 是 Windows 用戶安裝 Git 時默認安裝的命令行工具，大多數能替代 cmd 命令行。
...
3. 是架設在 mingw 上，所有指令類似 Linux 方式執行。開啟  就能在介面中，輸入類似 Linux 指令，(pwd, ls, ls -al, cd, ... 等)
...
1. Windows 安裝 [官方網站](https://golang.org/dl/)
2. 2020/05/29 go1.14.3.windows-amd64.msi => x86-64 => 115MB => SHA256 Checksum : c4f0be14d69f85a2bf4d4fbfd9c879c2f8fcb4560f117df0e17aa1999f8a319e
...
1. 安裝完 Golang 語言 安裝路徑 = GOROOT 路徑，也就是 存放 Golang 語言內建的程式庫所在位置。開發時 import 內建程式庫，不需要額外安裝。程式運行後，預設去 GOROOT 路徑下找尋 對應的 程式庫運行。

2. 查看 GOROOT 路徑，使用已下指令：
> go env<br>

會顯示當前 Golang 全部環境變數設定
> ...<br>
> set GOPATH=c:\users\user\go<br>
> set GOROOT=c:\go<br>
> ...<br>

3. 撰寫 Hello world 新增 hello.go 檔案
...
即可產生 hello.exe 檔案，並執行,即顯示結果 Hello, world
> hello
...
4. 若 import 別人開發第三方套件 (github.com/gin-gonic/gin) => Web Framework 不存在官方程式庫
...
1. GOPATH 就是存放 第三方套件
2. 設定 GOPATH 路徑 => 就是放置 Golang 專案的地方。
...
+ 假設 src 新增兩個資料夾 (mypkg 和 mytest)
    ...
    ```
    + 在 mytest.go 使用 mypkg 套件相關函式。而且透過 go install 指令將 mypkg 編譯成 .a file，在 mypkg 下執行指令 ``` go install ``` 會發現 GOPATH/pkg/windows_amd64/mypkg.a 檔案
    + GOOS 和 GOARCH => 組合成 windows_amd64
    + 若 ``` go install ``` 非在 GOPATH 路徑下，則先設定 GOBIN 環境變數，否則出錯
    + 通常 GOBIN 環境變數 => GOPATH/bin

### GOPATH 缺點

1. 第三方套件(不適官方程式庫) 需要放置在 GOPATH/src 路徑才能使用
2. 使用 ``` go get ``` 指令獲取 網路灀第三方套件，例如
```
go get github.com/gin-gonic/gin
```
這會使得 GOPATH/src 程式碼很複雜，除了自己的開發專案資料夾，也包含其他第三方程式庫。
3. 若開發專案 採用第三方套件是不同版本?? 以往解決方法是要設定多組不同 GOPATH。但網路上也有開發相對應的 Package manager，如 Vendor 解決
...
1. 官方在 1.11 推出 Go Modules 解決 第三方程式庫儲存在本地的空間，並給程式去引用
2. GO111MODULE 環境變數 有三種不同值，建議設定成 on
...
3. 採用 Go Modules 下載第三方套件 都會在哪呢 ? 其實在 GOPATH/pkg/mod 資料夾內
4. 簡易範例：
	+ 在 GOPATH/src 下建立一個專案資料夾 ```mkdir modtest```
    + 在該資料夾下執行 ```go mod init <module name>``` <module name> 可不填，直接預設專案資料夾名。執行後會看到 go.mod 檔案 內容如下：
    ```
	module modtest
    go 1.13
	```
...
5. 開啟 local 8080 port Web server 範例
...
1. ``` go env -w GOPRIVATE=gitlab.skyunion.net/rd1-gm ```
